...
def create_prompt(task: str, few_shot: int, **args):
    ...
    if args.get('sys_user', False):
        prompt += json.load(open(f"/content/llm-recipes-2/prompt/context.json"))[task] + "\n\n"

    module = __load_module_from_py_file(f"/content/llm-recipes-2/prompt/few_shot/{task}.py")
